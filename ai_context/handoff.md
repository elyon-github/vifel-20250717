...
| `CR-test` | (tip = this commit; run `git log --oneline -1`) | Client-requirements work branch. Clients kanban: badges are now buttons that open exactly the records they count, Pending shown first and loud, zero-value badges hidden, control-panel sort toggle (A → Z / Most Pending, ranked server-side by pending count — NOT lifetime transactions), Find Transfer + create-from-picker encoder navigation, Client Inventory Summary "Number of Pallets" column. Quant list renderer retired (column-squeeze handled in SCSS); its ui_test removed with it. |
