...
try:
    ...
    print('picking %s, line #%s (unmerged)' % (picking.name, line.x_studio_))
    ...
    # ---- real line merged immediately -------------------------------
    check('D5 real line is merged after Magic-Wizard initiation',
          line.is_pallet_merge and line.x_studio_pallet_series_id == adopted
          and line.result_package_id == tgt_pkg,
          (line.is_pallet_merge, line.x_studio_pallet_series_id))
    # ---- transient row synced (no divergence) -----------------------
    env.invalidate_all()
    tline = fw.line_ids.filtered(lambda l: l.stock_move_line == line.id)
    check('D6 transient row synced to the merge',
    ...
    env.flush_all()
    check('D11 un-merge from the Magic Wizard clears the real line flag',
          not line.is_pallet_merge)
    check('D12 transient row synced after un-merge',
          not tline2.is_pallet_merge)
