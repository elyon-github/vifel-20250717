...
const ch2 = [
  ...
  callout("Who does this · before you start", [
    ...
    "Have the client's merge details ready: whether they use one or more fixed pallets, or several special pallet types, and whether they track Lot No. / Batch #.",
  ]),
  ...
  body("There are two merge styles. The Multiple Special Pallet Support switch decides which one this client uses - follow Option A or Option B, not both."),

  P([T("Option A - Fixed Merge Pallet and PSI Support  (switch OFF)", { size: 22, bold: true, color: NAVY })],
    { spacing: { before: 120, after: 60 } }),
  body("Leave Multiple Special Pallet Support unticked. Every merge for this client is offered the client's dedicated Fixed pallets - now one OR MORE, each with its own PSI."),
  step(5, "Leave Multiple Special Pallet Support OFF"),
  step(6, "Add the Fixed Merge Pallets and their PSIs"),
  body("In the Fixed Merge Pallet and PSI Support table, click Add a line for each dedicated pallet: choose the pallet under Fixed Merge Pallet and type its series under Fixed Merge PSI. Add as many pallets as the client needs. A pallet must be empty and free (no stock, not reserved, not already a merge pallet) when you pin it, and each Fixed Merge PSI must be unique."),
  shot("02-05_fixed_mode.png"),
  caption("Figure 2.5 - Fixed Merge Pallet and PSI Support: one or more dedicated pallets, each with its own PSI."),

  P([T("Option B - Multiple Special Pallet Support  (switch ON)", { size: 22, bold: true, color: NAVY })],
    { spacing: { before: 160, after: 60 } }),
  body("Tick Multiple Special Pallet Support. The client's special pallet types appear in the Pallet Types table, each with its own prefix and running number (for example MDGM, SDMG, TDMG, BOC)."),
  step(5, "Tick Multiple Special Pallet Support"),
  ...
  step(7, "Set the DOCUMENTS options"),
...
const ch4 = [
  ...
  body("For a Fixed client, the dialog offers the client's dedicated Fixed pallet(s). If there is only one, it is already set to Merge Here; if the client has several, pick the one you want."),
  step(3, "Review and Confirm"),
  body("Make sure Merge Here is on for the fixed pallet you want (only one at a time), then click Confirm. If that pallet is still empty, this first line counts as one received pallet; later lines merging onto it (once it holds stock) do not."),
