...
class ClientTransferTypeWizard(models.TransientModel):
    # The _name keeps the multiple_relocation prefix even though the wizard now
    # lives in vifel_encoder_ux. Renaming it would rename the table and force
    # ir_model / ir_model_fields / ACL rows to be rebuilt on every database that
    # already has it, which buys nothing: a model's _name has never had to match
    # its module. Leave it alone.
