...
class ResPartnerClientButtons(models.Model):
    ...
    def _vifel_transfer_list(self, name, domain):
        ...
        tree_view = self.env.ref(
            'vifel_encoder_ux.view_picking_tree_vifel_client',
    ...
    def action_vifel_locations(self):
        ...
        search_view = self.env.ref(
            'vifel_encoder_ux.view_location_search_vifel_building',
            raise_if_not_found=False)
        if not search_view:
...
class IrActionsActWindowVifelClients(models.Model):
    """Default the Client tag on the Clients hub action.

    vifel_enable_quant_report_button used to sit beside this method. It stayed
    in multiple_relocation (models/quant_report_action.py) because the button
    it feeds lives there, so that module keeps working without this one.
    """
    ...
    def vifel_set_client_tag_default(self):
        ...
        """
        action = self.env.ref('vifel_encoder_ux.action_vifel_clients',
                              raise_if_not_found=False)
